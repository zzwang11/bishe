...
class MyMainWindow(QMainWindow, Ui_MainWindow):
    ...
    def writeconf(self):
        print(self.rere)
        li = self.get_con()
        self.wr = write_conf.writeThread('nsa',mod=li[0], add=li[1], ip=li[2], start=li[3], stop=li[4],
                                         ifband=li[5], averages=li[6], power=li[7], edelay=li[8], points=li[9],
                                         outputfile=li[10])
        self.wr.start()
        self.statusBar.showMessage('保存配置成功！', 10 ** 5)
    ...
    def readconf(self):
        directory1, filetype = QFileDialog.getOpenFileName(self,
                                                           ...
                                                           "./config/",
                                                           "Setting Files (*.ini)")
        self.rd = read_conf.MyTh('nsa', directory1)
        self.rd.myOut.connect(self.set_text)
        self.rd.start()
        self.statusBar.showMessage('从本地读取配置成功', 10 ** 4)
